...
BEGIN
SELECT logs.* from(
	SELECT ID as userid,UPPER(CONCAT(user.fname,' ',user.lname)) as name,dtr.datein,sched.id sched,DAYNAME(dtr.datein) dayname,
    CONCAT(
		COALESCE(
    ...
    IF(
        ...
        IF(
			dtr.datein >= "2020-03-18" and dtr.datein <= "2020-04-14",
            ( #gliding logs
				COALESCE(
					GREATEST(
            ...
            ( #normal logs
				COALESCE(
					GREATEST(
						TIMESTAMPDIFF(MINUTE,
							CONCAT(dtr.datein," ",IF(DAYNAME(dtr.datein) = "Monday","08:00:00",sched.am_in)), #08:00:00 1st condition timestamdiff default
							CONCAT(
								dtr.datein," ",
								COALESCE(
									(SELECT time from dtr_file dtr1 where
				...
				COALESCE(
					GREATEST(
						TIMESTAMPDIFF(MINUTE,
							CONCAT(dtr.datein," ",IF(DAYNAME(dtr.datein) = "Monday","13:00:00",sched.pm_in)), #13:00:00 1st condition timetamppdiff default
							CONCAT(
								dtr.datein," ",
								COALESCE(
									(SELECT time time from dtr_file dtr3 where
				...
				COALESCE(
					GREATEST(
						TIMESTAMPDIFF(MINUTE,
							...
							), #1st condition timedifference ruseltayong
							CONCAT(
								dtr.datein," ",sched.am_out #12:00:00
							) #2nd condition timedifference
						) #end time timedifference
				...
				COALESCE(
					GREATEST(
						TIMESTAMPDIFF(MINUTE,
							...
							CONCAT(
								dtr.datein," ",IF(DAYNAME(dtr.datein) = "Monday","17:00:00",sched.pm_out) #17:00:00 default
							)
						)
					,0)
				,0)
	...
	(select
        ...
        IF(
			...
			" ", #set empty if holiday,saturday and sunday
            IF(
				adddate('2010-01-01',t4.i*10000 + t3.i*1000 + t2.i*100 + t1.i*10 + t0.i) >= "2020-03-18" and adddate('2010-01-01',t4.i*10000 + t3.i*1000 + t2.i*100 + t1.i*10 + t0.i) <= "2020-04-14",
                ( #GLIDING LOGS
					COALESCE(
						GREATEST(
							TIMESTAMPDIFF(MINUTE,
