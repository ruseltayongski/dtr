        ...
        @foreach($document as $doc)
            ...
            if($doc->received_by!=0){
                ...
                if($released){
                    ...
                    if($released->status == 'report' || ($released->status == "waiting" && DocumentController::checkMinutes($released->released_date) > 30 ) ){
            ...
            <tr>
                ...
                <td class="trackFontSize {{ $received_success }}">
                    ...
                    @if($data['status'][$i]==1 && $i == count($data['received_by'])-1)
                        Cycle End
                    @else
                        {{ DocumentController::duration($start_date,$end_date) }}
                    @endif
                </td>
                <td class="trackFontSize {{ $received_success }}">{{ $data['remarks'][$i] }}</td>
                ...
                <td class="trackFontSize {{ $released_info }}">{{ $data['released_remarks'][$i] }}</td>
            </tr>
        @endfor
        </tbody>
    </table>
