...
SELECT DISTINCT e.userid, datein,holiday,remark, 
	...
	(SELECT  CONCAT(t3.time,'_',t3.edited) FROM dtr_file t3 WHERE userid = d.userid AND datein = d.datein and t3.time < '14:00:00' and t3.time > AMOUT and t3.time < PMOUT AND t3.event = 'IN' ORDER BY t3.time DESC LIMIT 1) as pm_in,
